...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    // ?sort=score returns the leaderboard (highest score first);
    // default stays newest-first for the live community feed.
    const sort = req.query?.sort === "score" ? "score.desc,created_at.desc" : "created_at.desc";
    const limitParam = parseInt(req.query?.limit, 10);
    const limit = Number.isFinite(limitParam) ? Math.max(1, Math.min(20, limitParam)) : 20;
    try {
      const url = `${SUPABASE_URL}/rest/v1/evaluations?select=name,snippet,score,level,created_at&order=${sort}&limit=${limit}`;
